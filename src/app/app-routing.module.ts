...
const eventDetailMatcher = (segments: UrlSegment[]): UrlMatchResult | null => {
  ...
  // Exclude known routes that shouldn't be matched here
  const excluded = ['payment', 'ticket', 'order-confirmation', 'orders', 'billet'];
  if (excluded.includes(segments[0].path)) {
    return null;
  }
...
const routes: Routes = [
  ...
  // Ajouté: routes events dans dashbord
  ...
  // ================== ROUTES ÉVÉNEMENTS (ajoutées) ==================
  // Specific routes BEFORE matcher (matchers have lower priority than fixed paths)
  {
