...
export class OrganisateurPageComponent implements OnInit, OnDestroy {
  ...
  refreshWeatherAndInsights(): void {
    ...
    if (canUseForecast) {
      // Extract only the date part (YYYY-MM-DD) from startDate which may include time
      const dateOnly = this.form.startDate.slice(0, 10);
      this.weatherService.getForecastByCoordsAtDate(lat, lng, dateOnly).subscribe({
