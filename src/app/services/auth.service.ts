...
export class AuthService {
  ...
  login(payload: LoginRequest): Observable<AuthResponse> {
    return this.http.post<ApiResponse<AuthResponse>>(`${this.authApiUrl}/login`, payload)
      .pipe(
  ...
  register(payload: RegisterRequest): Observable<AuthResponse> {
    return this.http.post<ApiResponse<AuthResponse>>(`${this.authApiUrl}/register`, payload)
      .pipe(
        map((response) => response.data),
        tap((auth) => this.persistAuth(auth))
      );
  ...
  forgotPassword(payload: ForgotPasswordRequest): Observable<void> {
    return this.http.post<ApiResponse<null>>(`${this.authApiUrl}/forgot-password`, payload)
      .pipe(map(() => void 0));
  }

  resetPassword(payload: ResetPasswordRequest): Observable<void> {
    return this.http.post<ApiResponse<null>>(`${this.authApiUrl}/reset-password`, payload)
      .pipe(map(() => void 0));
  }

  updateCurrentUserProfile(payload: UpdateCurrentUserRequest): Observable<UserResponse> {
    const currentUser = this.getCurrentUser();
    if (!currentUser?.id) throw new Error('Utilisateur non connecte');
    return this.http
      .put<ApiResponse<UserResponse>>(`${this.usersApiUrl}/${currentUser.id}`, payload)
      .pipe(
        map((response) => response.data),
        tap((updatedUser) => localStorage.setItem(this.userStorageKey, JSON.stringify(updatedUser)))
      );
  }
