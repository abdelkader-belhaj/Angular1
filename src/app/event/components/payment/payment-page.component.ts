...
export class PaymentPageComponent implements OnInit {
  ...
  ngOnInit(): void {
    // For lazy-loaded modules with path:'', params live on the parent route
    const paramMap = this.route.snapshot.paramMap.get('reservationId')
      ? this.route.snapshot.paramMap
      : this.route.parent?.snapshot.paramMap ?? this.route.snapshot.paramMap;

    const queryParamMap = this.route.snapshot.queryParamMap.get('stripeSessionId')
      ? this.route.snapshot.queryParamMap
      : this.route.parent?.snapshot.queryParamMap ?? this.route.snapshot.queryParamMap;

    const id = Number(
      paramMap.get('reservationId') ??
      queryParamMap.get('reservationId')
    );

    this.stripeSessionId = queryParamMap.get('stripeSessionId') ?? '';
  ...
  private confirmStripeReturn(): void {
    console.log('stripeSessionId:', this.stripeSessionId);
    console.log('reservationId:', this.reservation?.id);
    ...
    this.paymentService.confirmStripeSession({
      ...
      next: () => {
        ...
        this.resService.getById(this.reservation.id).subscribe({
          next: (updatedReservation: EventReservation) => {
            ...
            // Auto-redirect to reservations after 2 seconds to show success message
            setTimeout(() => {
          ...
          error: () => {
            ...
            // Auto-redirect to reservations after 2 seconds even if fetch fails
            setTimeout(() => {
              void this.router.navigate(['/mes-reservations-event'], {
                queryParams: { payment: 'success' },
              });
      ...
      error: (err) => {
        ...
        // Fallback redirect on error after 2 seconds
        setTimeout(() => {
          void this.router.navigate(['/mes-reservations-event']);
        }, 2000);
      },
